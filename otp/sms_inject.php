...
#        echo "<pre>Destination : $this->dest\nSender : $sender\nMessage :\n";print_r($this->msg_part);
    ...
    private function inject($sender='')
    {
        ...
        foreach($this->msg_part as $number => $sms)
        {
            if($number==1)
            {
                $query = $this->res->prepare("insert into outbox (`UDH`,`DestinationNumber`,`TextDecoded`,`MultiPart`,`SenderID`) values (:udh, :dest, :msg, :multi, :sender)");
                $query->execute(array("udh" => $sms['udh'], "dest" => $this->dest, "msg" => $sms['msg'], "multi" => $multipart, "sender" => $sender)) or die("SQL fehlgeschlagen: ". print_r($query->errorInfo(), true));
                $id=$this->res->lastInsertId();
            }
            else
            {
                $query = $this->res->prepare("insert into outbox_multipart (`UDH`,`SequencePosition`,`TextDecoded`,`ID`) values (:udh, :number, :msg, :id)");
                $query->execute(array("udh" => $sms['udh'], "number" => $number, "msg" => $sms['msg'], "id" => $id)) or die ("SQL fehlgeschlagen: ".print_r($query->errorInfo(),true));
            }
        }
    }
